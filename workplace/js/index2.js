...
async function getchartInfo(listName,claList){
  var bookname=["To Kill a Mockingbird", "Pride and Prejudice", "Lebs", "The Swan Book", "The Big Sleep", "Gone Girl", "Lord of the Rings: Fellowship of the Ring", "The Fifth Season", "Eleanor & Park", "Harry Potter and the Philosopher's Stone"];
...
async function getBookInfo(size){
  var size = 10;
  var n=1;
...
async function getvalue(){
  var all_infor =await  getBookInfo(10);
  console.log("all",all_infor);
  creatTableString(all_infor );

  }
  // for (var i = 10; i > 0; i--) {
  //   book = all_infor ["book"+i];
  //   author = all_infor["author"+i];
  //   genre=all_infor["genre"+i] ;
  //   Q1=all_infor ["Q1"+i];
  //   Q2=all_infor ["Q2"+i];
  //   Q3=all_infor ["Q3"+i];
...
// Generate comments in a loop, index is the name of Div and data is audio URL
// function creatTableString(index,book,author,genre,Q1,Q2,Q3){

function deletdata(notification_id){
  console.log(notification_id);
  var action = "delete";
  Getstring(action,notification_id);  
  }




function creatTableString( ){
  all_infor= Getstring("read",'');
  for (var i = 1; i < 11; i++) {
  var index=i
  ...
  console.log (i,book,author,genre,Q1,Q2,Q3);
  var string =' <td ><div class="custom-control custom-checkbox"> <input name="single_chose" class="custom-control-input" type="radio" id="'+index+'"> <label class="custom-control-label" for="'+index+'"></label></div></td> <td name="input_bookname" >'+book+'</td><td name="Author">'+author+'</td><td name="Genre">'+genre+'</td><td name="Q1">'+Q1+'</td><td name="Q2">'+Q2+'</td><td name="Q3">'+Q3+'</td>'
  var id = "test"+i;
  // console.log(id);
...
function Getstring(action,id){ 
  var all_infor={
    ...
    if (action ==='read') {return all_infor;}
          ...
          delete all_infor["book" + id];
          delete all_infor["author" + id];
          delete all_infor["genre" + id];
          delete all_infor["Q1" + id];
          delete all_infor["Q2" + id];
          delete all_infor["Q3" + id];
          delete all_infor["n" + id];
          console.log(all_infor);
          for (var i = 1; i < 10; i++) {
          var index=i
          var book =  all_infor ["book"+i];
          var author = all_infor["author"+i];
